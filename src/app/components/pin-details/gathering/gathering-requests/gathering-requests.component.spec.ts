...
describe('GatheringRequestsComponent', () => {
  ...
  let el;
  let mockGroupService, mockBlandPageService, mockStateService;
  ...
  beforeEach(() => {
    ...
    mockGroupService = jasmine.createSpyObj<GroupInquiryService>('groupService', [
      'getGroupRequests',
      'acceptOrDenyRequest'
    ]);

    TestBed.configureTestingModule({
      declarations: [GatheringRequestsComponent],
      imports: [],
      providers: [
  ...
  it('should init and filter out placed inquiries', () => {
    (<jasmine.Spy>mockGroupService.getGroupRequests).and.returnValue(
      Observable.of([{ inquiryId: 1 }, { inquiryId: 2 }, { inquiryId: 3, placed: true }])
    );
  ...
  it('should accept Invitation', () => {
    ...
    expect(mockGroupService.acceptOrDenyRequest).toHaveBeenCalledWith(
      comp.pin.gathering.groupId,
      comp.pin.gathering.groupTypeId,
      true,
      inquiry
    );
