describe('try-group-request-success.component', () => {
  it('Navigates to the group details when close ("x") is clicked', () => {
    comp.onClose();
    expect(comp.route.navigate).toHaveBeenCalledWith([`/small-group/${groupId}`]);
  });
  ...
  it('Navigates to search when "Find another group" is clicked', () => {
    ...
    expect(comp.route.navigate).toHaveBeenCalledWith([`/`]);
  });
});
